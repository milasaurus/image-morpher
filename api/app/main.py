...
async def post_write_instruction(req: WriteInstructionRequest):
    try:
        choice = await write_instruction(
            req.prompt, req.winner_url, req.strategy, req.previous_instructions
        )
        return WriteInstructionResponse(instruction=choice.instruction)
...
async def post_generate(req: GenerateRequest):
    try:
        if req.strategy in ("tweak", "preserve_subject"):
...
async def post_round(req: RoundRequest):
    try:
        ...
        return RoundResponse(images=[new_b], instruction=choice.instruction, strategy=req.strategy)
